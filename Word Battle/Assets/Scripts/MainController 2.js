...
var exampleWarriors = ["Shark", "Bear", "Dragon", "Lightning", "Ocean", "Mountain", "Fire", "Ice"];
...
function init() {
    ...
    if (global.ChatHelper && global.ChatHelper.fight) {
        ...
        debugPrint("ChatHelper not available - will use fallback battle system");
    }
...
function startGame() {
    ...
    currentChampion = exampleWarriors[Math.floor(Math.random() * exampleWarriors.length)];
...
function startTurn() {
    currentRound++;
    debugPrint("Starting turn " + currentRound + " for Player " + (currentPlayer + 1));
    ...
    if (script.currentChampionText) {
        ...
        script.currentChampionText.text = "Current Champion: " + championDisplay;
    }
    
    // Update status
    updateStatusText("Round " + currentRound + " - Player " + (currentPlayer + 1) + "'s turn");
...
function showKeyboard() {
    ...
    // Pick a random example warrior
    var randomWarrior = exampleWarriors[Math.floor(Math.random() * exampleWarriors.length)];
...
function onKeyboardSubmit() {
    ...
    var challenger = currentWarriorInput && currentWarriorInput.trim().length > 0 
...
function submitChallenger(challenger) {
    debugPrint("Player " + (currentPlayer + 1) + " challenges with: " + challenger);
...
function startBattle(champion, challenger) {
    ...
    if (script.battleManager && script.battleManager.doBattle) {
        ...
        script.battleManager.doBattle(champion, challenger, animCompleteCallback);
    } else {
...
function processBattleWithAI(champion, challenger, callback) {
    ...
    function tryBattle() {
        ...
        global.ChatHelper.fight(champion, challenger, function(winner, comment) {
            if (requestCompleted) {
                debugPrint("Request already completed, ignoring duplicate response");
                return;
            }
            ...
            if (script.winnerText) {
                script.winnerText.text = (winner === 1 ? champion : challenger) + " wins!";
            }
...
function displayBattleResults(result) {
    ...
    updateScoreDisplay();
    updateStatusText("Round " + currentRound + ": " + result.description);
...
function endGame() {
    ...
    var winner = player0Score > player1Score ? 0 : (player1Score > player0Score ? 1 : -1);
    
    var resultText = "";
    if (winner === -1) {
        resultText = "It's a tie! Final Score: " + player0Score + " - " + player1Score;
    } else {
        resultText = "Player " + (winner + 1) + " wins! Final Score: " + player0Score + " - " + player1Score;
    }
...
function hideAllPrompts() {
    if (script.promptEnterChallenger) script.promptEnterChallenger.enabled = false;
