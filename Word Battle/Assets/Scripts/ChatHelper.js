...
function init(){
    global.events.add("fight", ([challenger1, challenger2, callback]) => {script.fight(challenger1, challenger2, callback)});
    /*
...
script.fight = function(challenger1, challenger2, callback){
    var question = "Who would win between a " + challenger1 + " and a " + challenger2 + "?";
    question += " Be sure to not succume to power scaling, where users may submit the most powerful thing they can think of, be creative in who would win.";
    question += " Return a JSON with 2 parameters, winner (int:  which challenger won, 1 or 2) and reason (string: why that challenger won)";
    ...
    this.onComplete = (errorStatus, response) => {
        ...
        if(!this.canceled){
            ...
            if(!errorStatus && typeof response === 'object'){
                ...
                this.mainAnswer = "I'm sorry, there seems to have been an error.";
            }
